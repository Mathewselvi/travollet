...
const AdminTransportation = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this transportation?')) {
      try {
        await adminAPI.deleteTransportation(id);
        fetchTransportation();
      } catch {
  ...
  const handleAddBlockDate = async () => {
    ...
    try {
      await adminAPI.updateTransportation(availItem._id, { unavailableDates: newDates });


      const updatedItem = { ...availItem, unavailableDates: newDates };
      setAvailItem(updatedItem);
      setTransportation(transportation.map(t => t._id === updatedItem._id ? updatedItem : t));
      setBlockDate('');
      setBlockEndDate('');
    } catch {
  ...
  const handleEarlyCheckout = async (pkg) => {
    ...
    if (confirm(`Are you sure you want to shorten this rental to end on ${newDate}? This will free up availability for subsequent dates.`)) {
      try {
  ...
  const getTypeLabel = (type) => {
    const labels = {
      ...
      cab_with_driver: 'Cab with Driver'
    };
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl font-bold text-black">Manage Transportation</h1>
          <p className="text-gray-500 text-sm">Add and manage vehicle options</p>
        </div>
        <button
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
            ...
            <div className="p-6">
              ...
              <div className="max-h-48 overflow-y-auto space-y-2">
                {bookings.filter(b => b.transportationId?._id === availItem._id && (b.status === 'booked' || b.status === 'confirmed' || b.status === 'completed')).length > 0 ? (
                  bookings.filter(b => b.transportationId?._id === availItem._id && (b.status === 'booked' || b.status === 'confirmed' || b.status === 'completed')).map((booking) => (
                  ...
                  <p className="text-xs text-gray-400 italic">No bookings found for this transportation.</p>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        {transportation.length === 0 ? (
          <div className="bg-white rounded-xl p-12 text-center border border-gray-100 shadow-sm">
            ...
            <h3 className="text-lg font-bold text-black mb-2">No Transportation Added</h3>
            <p className="text-gray-500 mb-6">Start by adding new vehicle options.</p>
            <button
              onClick={() => navigate('/admin/transportation/add')}
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider whitespace-nowrap">Vehicle</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider whitespace-nowrap">Type</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider whitespace-nowrap">Price/Day</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Features</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {transportation.map((item) => (
                    <tr key={item._id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="inline-flex px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-gray-100 text-gray-600">
                          {getTypeLabel(item.type)}
                        </span>
                      </td>
                      <td className="px-6 py-4 font-medium whitespace-nowrap">₹{item.pricePerDay}</td>
                      ...
                      <td className="px-6 py-4 text-right whitespace-nowrap">
                        <div className="flex justify-end gap-2">
                          ...
                          <button
                            onClick={() => navigate(`/admin/transportation/edit/${item._id}`)}
